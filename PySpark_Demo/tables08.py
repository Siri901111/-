...
# 1.创建SparkSession
spark = SparkSession.builder.appName("tables08_task").getOrCreate()
...
tables08_sql = '''
    SELECT 
        ...
        COUNT(CASE WHEN ontime_rate >= 90 THEN 1 END) as high_ontime_flights,
        COUNT(CASE WHEN ontime_rate < 90 AND ontime_rate >= 70 THEN 1 END) as mid_ontime_flights,
        COUNT(CASE WHEN ontime_rate < 70 THEN 1 END) as low_ontime_flights
    FROM ods_flight
    GROUP BY stime
    ORDER BY stime
'''
...
# 执行SQL并显示结果
spark.sql(tables08_sql).show()

# 将结果写入MySQL
spark.sql(tables08_sql).write.jdbc(mysql_url, 'tables08', 'overwrite', prop) 