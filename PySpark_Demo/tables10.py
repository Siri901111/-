...
from pyspark.sql.functions import col, count, avg, collect_set
...
# 1.创建SparkSession
spark = SparkSession.builder.appName("tables10_task").getOrCreate()
...
tables10_sql = '''
    SELECT 
        ...
        COUNT(DISTINCT CONCAT(start_city, '-', end_city)) as route_count,
        COUNT(DISTINCT start_city) as city_coverage,
...
# 执行SQL并显示结果
spark.sql(tables10_sql).show()

# 将结果写入MySQL
spark.sql(tables10_sql).write.jdbc(mysql_url, 'tables10', 'overwrite', prop) 