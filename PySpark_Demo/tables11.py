...
# 1.创建SparkSession
spark = SparkSession.builder.appName("tables11_task").getOrCreate()
...
tables11_sql = '''
    SELECT 
        ...
        COUNT(DISTINCT end_city) as dest_city_count
    FROM ods_flight
    WHERE flight_type1 IS NOT NULL
    GROUP BY flight_type1, start_city
    ORDER BY flight_type1, flight_count DESC
'''

# 执行SQL并显示结果
spark.sql(tables11_sql).show()

# 将结果写入MySQL
spark.sql(tables11_sql).write.jdbc(mysql_url, 'tables11', 'overwrite', prop) 