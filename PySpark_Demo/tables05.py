...
# 1.创建SparkSession
spark = SparkSession.builder.appName("tables05_task").getOrCreate()
...
tables05_sql='''
                        ...
                        select flight_company,count(distinct airline_name) num
                        from ods_flight
...
'''
spark.sql(tables05_sql).show()
spark.sql(tables05_sql).write.jdbc(mysql_url, 'tables05', 'overwrite', prop)